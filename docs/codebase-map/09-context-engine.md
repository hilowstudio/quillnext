...
| `MasterContextParams.courseBlockId/bookId/videoId/articleId/documentId` | UNFINISHED (fields) | Declared (master-context.ts:13-18), threaded `GeneratorForm`→`generateLearningTool`→`getMasterContext`/`buildMasterPrompt` + inspect route, but never **read** by any sub-fetcher — the unbuilt context-injection half of source-anchored generation (the lineage half, `generatedFrom*`, is live). Q-09-005 OPEN. |
