...
Q-01-004  [LOW]  Lint rules downgraded to warnings hide debt — eslint.config.mjs (ratchet block ~:20-40)  ·  Reviewed 2026-06-19 (Session 1): kept OPEN (owner). **Burndown pass 1, 2026-06-23 (later):** corrected the baseline (eslint was double-counting a stale leftover `.claude/worktrees/` Workflow worktree → added it to `ignores`), then **637 → 397 warnings / 0 errors** (518 after lock passes 1 + 2; 397 after the `as any` cast burn-down — see Evidence), with **11 rules burned to 0 and LOCKED warn→error** — pass 1: ban-ts-comment, no-empty-object-type, no-require-imports, no-wrapper-object-types, prefer-const, react/no-unescaped-entities, jsx-a11y/alt-text, import/no-anonymous-default-export; pass 2 (react-hooks): error-boundaries, set-state-in-effect, exhaustive-deps. It is a deliberate, commented lint-adoption ratchet — new violations of the LOCKED rules now fail CI — not a defect. **2026-06-25: both `@typescript-eslint/no-explicit-any` AND `@typescript-eslint/no-unused-vars` LOCKED warn→error — all 13 ratchet rules now locked; Q-01-004 RESOLVED.** See CHANGELOG.md (2026-06-23 passes 1 + 2; 2026-06-24/25 the no-explicit-any phases; 2026-06-25 the no-unused-vars final pass).
  Evidence: **no-explicit-any reached 0 and is LOCKED warn→error (2026-06-25).** Full burndown 273→0: the `as any` CAST phase (273→153, waves 1–17, **0 casts in src** + a surfaced AI SDK v4→v5 migration) then the `: any` ANNOTATION phase (153→0, 2026-06-24/25) — which surfaced + fixed several real bugs the `any` was hiding (Living Library description/generationContext never selected; planner smart-slot read video.url/doc.title that don't exist; onboarding off-days never round-tripped; transcript course narratives never rendered) and corrected an earlier laundering slip (external `response.json()` boundaries now Zod-validated, not asserted — see the any-annotation-burndown-approach memory). 2 benign `$ZodType<any,any>` bounds accepted via disable-with-reason. **`no-unused-vars` then reached 0 (2026-06-25): 230→0 across 80 files** — config gained `^_` ignore patterns; the bug-prone write-only bucket was hand-cleared (surfacing+fixing the PlannerGrid cross-student-drag bug, the ResourcePicker loading-state mis-wire, a broken FILE-upload flag, a trimmed half-built BibleAudioPlayer scrubber) and the mechanical bulk swept by 5 audited parallel agents. Still at `"warn"` by design: no-img-element 11 (all remote/data-URL → intentionally left). The react-hooks trio (error-boundaries/set-state-in-effect/exhaustive-deps) was resolved in pass 2 per the owner's React-Compiler-era guidance: real fixes where safe (error.tsx boundaries, effect-as-event→handler, watch() hoist) + reasoned scoped suppressions for genuine reset/sync/footgun cases (6 disables, each with a reason).
  Impact: Pervasive existing violations of the still-warn rules don't fail CI; type-safety regressions can land as warnings. (Intentional per comment; all 13 ratchet rules are now enforced.)
  Status: ✅ RESOLVED 2026-06-25 — the lint-debt ratchet is COMPLETE. **All 13 rules now LOCKED warn→error**: the 11 from passes 1+2 (2026-06-23) + **no-explicit-any** (273→0, the as-any cast + `: any` annotation phases) + **no-unused-vars** (230→0 across 80 files, 2026-06-25). `no-img-element` (11) stays warn by design (remote/data-URL images bypass next/image). New violations of any locked rule now fail CI; residual disables-with-reason are the accepted end state. All burndown commits pushed to `main`.
