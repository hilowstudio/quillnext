...
Q-01-004  [LOW]  Lint rules downgraded to warnings hide debt — eslint.config.mjs:24-34  ·  Reviewed 2026-06-19 (Session 1): kept OPEN (owner). `eslint .` = 687 warnings / 0 errors back the 9 downgrades (top: no-explicit-any 289, no-unescaped-entities 58, error-boundaries 17; tail: prefer-const 7, no-empty-object-type 3, no-require-imports 2). It is a deliberate, commented lint-adoption ratchet — new violations of enforced rules still fail CI — not a defect. See CHANGELOG.md.
  Evidence: 9 rules set to `"warn"` incl. `@typescript-eslint/no-explicit-any`, `ban-ts-comment`, `no-require-imports`, `react-hooks/set-state-in-effect`, `prefer-const`.
  Impact: Pervasive existing violations don't fail CI; type-safety / hooks-correctness regressions can land as warnings. (Intentional per comment, but masks scope of debt.)
  Status: documented (not fixed) — OPEN [LOW], owner-reviewed 2026-06-19
