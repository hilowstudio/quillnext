...
> 1. **Q-01-004** (ch.01) — lint rules downgraded to warnings; owner-accepted deliberate adoption ratchet → **kept-OPEN (owner)**. *(Burndown 2026-06-23: baseline corrected for a stale `.claude/worktrees/` lint-pollution; **11 rules locked warn→error** (passes 1+2, incl. the react-hooks trio); then an in-progress `as any` cast burn-down (waves 1-2b, commits 264bef4/9f3f93c/939b8db) took no-explicit-any **273→234** and total **637→479**; remaining Tier C [no-explicit-any 234 (~71 casts left), no-unused-vars 234 — the LAST pass] + no-img-element 11 (by design) — see CHANGELOG + the as-any-burndown-approach memory.)*
