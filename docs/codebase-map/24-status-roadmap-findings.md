...
> 1. **Q-01-004** (ch.01) — lint rules downgraded to warnings; owner-accepted deliberate adoption ratchet → **kept-OPEN (owner)**. *(Burndown 2026-06-23: baseline corrected for a stale `.claude/worktrees/` lint-pollution; **11 rules locked warn→error** (passes 1+2, incl. the react-hooks trio); then the `as any` cast burn-down COMPLETED (waves 1–17, 2026-06-24): no-explicit-any **273→153** and total **637→397**, **0 casts in src** (and it triggered a full AI SDK v4→v5 migration); remaining Tier C = the residual 153 `: any` ANNOTATIONS + the **no-unused-vars 234** FINAL pass + no-img-element 11 (by design) — see CHANGELOG + the as-any-burndown-approach memory.)*
