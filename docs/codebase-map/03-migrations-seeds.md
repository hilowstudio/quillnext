...
| `seed-book.ts` | One test Book ("The Hobbit"). | NOT wired (no script) — DEAD + broken |
...
| `seed.ts` | DONE | `db:seed` (package.json:17); idempotent (seed.ts:96-98, upserts) |
| `seed-generator-content-types.ts` | DONE | `db:seed:generators` (:18); idempotent via delete+upsert (line 42,164) |
...
| `seed-book.ts` | DEAD + broken | no package.json script (verified grep); `new PrismaClient()` with NO adapter (line 13) fails under Prisma 7 |
...
- **Prisma models written**: Subject, Strand, Topic, Subtopic, Objective, GradeBand (seed.ts); ResourceKind (generators); County (counties); Devotional (discipleship); Catechism, CatechismQuestion (catechisms); CommentaryChapter, CommentarySection (commentary); Book (seed-book, dead).
...
Q-03-001  [MED]  `seed-book.ts` is broken under Prisma 7 and dead-wired  — prisma/seed-book.ts:13
...
Q-03-003  [MED]  Seeders connect directly (likely bypass-RLS role) and write global tables with SSL verification disabled  — prisma/seed.ts:18-21, seed-catechisms.ts:21, seed-counties.ts:14
  ...
  Impact: Expected for global reference data, but `rejectUnauthorized:false` disables TLS cert validation (MITM exposure during seeding) and the direct role has broad write power; worth noting as a posture risk.
  Status: documented (not fixed)

Q-03-004  [LOW]  `seed.ts` spine upsert silently de-dupes on `objective.code` but updates only `text`, not grade/complexity/sortOrder  — prisma/seed.ts:208-221
  Evidence: the Objective upsert `update` block sets only `text`; gradeLevel/complexity are applied later by the sequenced pass (seed.ts:295-300), and `sortOrder` is hard-coded `0` on create with a comment "Will be updated from sequenced data" — but no code ever updates `sortOrder`.
  Impact: All spine `sortOrder` values stay 0 (the master JSON ordering is lost); any UI relying on `sortOrder` gets undefined ordering. Subject/Strand/Topic/Subtopic also created with `sortOrder: 0`.
  Status: documented (not fixed)

Q-03-005  [LOW]  `seed-generator-content-types.ts` does an unconditional destructive `deleteMany` of all ResourceKinds  — prisma/seed-generator-content-types.ts:42
  Evidence: `await prisma.resourceKind.deleteMany({})` runs every invocation before re-seeding; comment (lines 39-41) admits it is unsafe once Resource/BookGeneratedMaterial rows reference ResourceKinds.
  Impact: FK `resource_kinds` references (resources.resource_kind_id is RESTRICT in init:1386) would block the delete on a populated DB, OR (if it succeeds early) wipe the generator catalog. Idempotent only on a pristine DB.
  Status: documented (not fixed)
