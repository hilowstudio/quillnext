...
- [x] **SEC-10 — Add Postgres RLS policies + a non-superuser app role (close the RLS-theater gap)** (P0/P1, L, security-idor)
  - **✅ DONE — RLS is real and cut over in prod (owner-confirmed 2026-06-17).** The "theater" described below no longer matches `main`: there are **58 `CREATE POLICY` statements across 8 migrations** (core `prisma/migrations/00000000000002_rls_policies/migration.sql` + per-feature policy migrations), plus a per-request GUC-stamping data layer in `src/server/db.ts` — a `$extends` query extension that wraps every org-scoped op in `set_config('app.current_org'/'app.current_user', …, true)`, a `withTenant()` helper for `$transaction`/raw SQL (wired into **51 files**), and a `CONTEXT_FREE_MODELS` allow-list for auth + global-reference tables. The app runs as a dedicated **non-BYPASSRLS `app_user`** role behind the `RLS_ENABLED` flag (`db.ts:9-11`); **prod sets `RLS_ENABLED=true` and connects as `app_user`**. DB-level tenant isolation is live; SEC-1…SEC-9's app-level `where:{organizationId}` fixes now sit on top of it as defense-in-depth.
  - **Residual (NOT this task — tracked at HYG-1/HYG-2):** automated two-org isolation tests + a static lint guard for any `db.*.find*` on a tenant model missing an `organizationId` filter, and the conclusive runtime check (as `app_user`, a cross-tenant `SELECT` returns 0 rows) once the test harness exists.
  - **Problem (RESOLVED — describes the pre-cutover audit state):** RLS was *enabled* on every table by the `rls_auto_enable` trigger, but the repo had **zero `CREATE POLICY`** statements and the app connected as the Supabase `postgres` superuser (BYPASSRLS). RLS was a no-op; isolation was 100% application-level `where:{ organizationId }`. Any forgotten filter (see SEC-1…SEC-6) leaked across tenants, and the "RLS enabled" flag falsely reassured.
...
> **⭐ PROFILES EPIC (foundational — supersedes FEAT-2).** A Netflix-style profile system: account
> login → "Who's learning today?" picker → typed **PARENT/STUDENT** profiles with **per-profile PINs**;
> a learner can be a child *or* an adult ("My Learning"); a `viewMode` seam for a future kid-view.
> Design spec: [`docs/superpowers/specs/2026-06-18-profile-system-design.md`](../superpowers/specs/2026-06-18-profile-system-design.md).
> Model: `User` (auth) / `Profile` (typed, PIN'd identity) / `Learner` (rename of `Student`).
> **Build Phase A (slices 1–5: model+migration, session, picker, type-authz, PIN) before** FEAT-19,
> FEAT-20, and the `/student/dashboard` divergence (HYG-52) — those then become "render the active
> profile's context." This epic **supersedes FEAT-2**, **absorbs FEAT-22** (PIN re-entry on edit) and
> **HYG-12** (PIN hash in export — the PIN moves off `ClassroomInstructor` onto `Profile`). Phase B
> (slice 6: parent-as-learner + kid seam) follows. Orthogonal work (BUG-\*, grounding, hygiene) runs
> in parallel.

- [x] **FEAT-1 — Repoint or remove the phantom `/dashboard` guard in `proxy.ts`** (P1, M, BUILD-or-REMOVE)
  - **✅ DONE (2026-06-17) — BUILT a real central guard (public allow-list).** `src/proxy.ts` (the live Next 16 `proxy` middleware) now allow-lists the public routes `{/login, /signup, /privacy, /terms, /about, /changelog}` and redirects every other route to `/login` when there's no session — so a page that forgets its own `auth()` is now **fail-closed** (centrally covers `/courses/new`, `/students/new`, `/students/[id]/family-discipleship/*`, etc.). The dead `/dashboard` branch is gone; the `/onboarding` guard is subsumed by the catch-all; the matcher also excludes `/assets` (login logo no longer redirected, and `auth()` no longer runs per static-asset request). **Owner product decision:** the **entire `/family-discipleship` subtree is now gated behind login** — this resolves **HYG-18/19** toward "all read content requires auth" (the previously-public index/catechism/devotionals/missions/neighbor are now login-gated). The dead `authorized()` callback was removed from `src/auth.config.ts` and the now-vestigial `...authConfig.callbacks` spread from `src/auth.ts` (the **HYG-49** slice for these files). **Verified:** `tsc --noEmit` clean; live logged-out smoke test → `/courses/new`, `/students/new`, `/family-discipleship/catechism` all `307 → /login`, `/assets/branding/Quill-and-Compass.png` `200`. (The `/login`/`/privacy` 500s seen during testing are an **unrelated local-dev environment failure**, NOT this change: Turbopack panics spawning the PostCSS worker for `src/app/globals.css` — Windows exit `0xc0000142` / STATUS_DLL_INIT_FAILED — which blocked **all** local page rendering regardless of route (root-caused to a stray home-dir `package-lock.json` forcing the wrong workspace root — since removed; Turbopack now renders all pages `200`). The proxy itself runs (`proxy.ts: 7ms`, passes public routes through) and `tsc` is clean; prod on Vercel/Linux is unaffected.)
  - **Runbook corrections found here:** the old "Depends on: HYG-9 (delete the dead `authorized()` callback)" was a **wrong cross-ref** — that cleanup is **HYG-49**; HYG-9 is the Supabase MCP `project_ref` task. The leftover `// ... (auth checks) ...` placeholder still sits at `src/app/page.tsx:16` (a HYG-52 item). HYG-5 (proxy edge-vs-Node runtime) is unaffected and still open.
  - **Problem (RESOLVED):** `proxy.ts` (and the dead `authorized()` callback) only positively guard `/dashboard`, but **no `/dashboard` route exists** (real home is `/`, student view `/student/dashboard`). The proxy is a no-op for security; any new page that forgets its own `auth()` is silently public.
...
- [ ] **FEAT-2 — ➡️ SUPERSEDED by the Profiles epic** (was: enforce the instructor PIN gate) (P1, L) — the never-verified PIN is replaced by **per-profile PINs** built in **Phase A** of the Profiles epic (see the callout at the top of Phase 1 + the design spec). Don't build this standalone. Original problem retained below for history.
...
- [x] **FEAT-26 — Verify/configure the Resend sender domain so caregiver safety emails actually deliver** (P1, S, infra-ci / child-safety)
  - **✅ DONE (2026-06-18):** `quillandcompass.app` is now **verified** in Resend (SPF MX + SPF TXT + DKIM `resend._domainkey` all `verified`, confirmed via the Resend domains API). With `SAFETY_ALERT_FROM="Quill & Compass Safety <safety@quillandcompass.app>"` sending from a verified domain, `resend.emails.send` now succeeds. **Verified end-to-end:** the 3 previously-undelivered `PARENT_SUMMARY_URGENT` flags were re-sent via the real `sendSafetyAlert` → Resend accepted (no error), delivered to the caregiver, and `alertSent` flipped to `true` (DB now: 3 parent-summary flags / 3 sent / 0 unsent).
  - **Residual (optional hardening, NOT done — not blocking):** (1) confirm `RESEND_API_KEY` + `SAFETY_ALERT_FROM` are set in the **Vercel PROD** env (verified in local `.env` only; domain verification is account-level so it applies regardless); (2) the Resend smoke-test/health check and (3) the fallback alert channel (Slack/webhook when `sendSafetyAlert` returns `sent === false`) from the original fix list were not built — spin off as a separate item if wanted.
  - **Problem (RESOLVED):** **The single biggest safety-delivery risk.** `sendSafetyAlert` only flips `SafetyFlag.alertSent = true` after Resend confirms delivery. `from` falls back to Resend's test sender `onboarding@resend.dev` when `SAFETY_ALERT_FROM` is unset (delivers ONLY to the Resend account owner). `.env:55` does set `SAFETY_ALERT_FROM="Quill & Compass Safety <safety@quillandcompass.app>"`, but **if the `quillandcompass.app` domain is not verified in the Resend dashboard, `resend.emails.send` returns an error and `alertSent` stays `false`** — i.e. a detected self-harm/abuse concern produces NO delivered caregiver alert, silently. This is an env/ops/deliverability item, not just the cosmetic docstring-TLD fix folded into HYG-37.
...
- Resend caregiver-safety email deliverability (verify `quillandcompass.app` sender domain) → **FEAT-26** (doc 20) — **✅ DONE 2026-06-18** (domain verified in Resend; delivery confirmed end-to-end). HYG-37 (cosmetic `safety-alert.ts:22` docstring TLD) still open.
