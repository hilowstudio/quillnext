...
- `registry.ts` — **BY-TITLE**. Resolves ONE named work to ONE concatenated full text. A `TextSource` (registry.ts:72-76) is `{ key, discover(meta)→{sourceId,textUrl}|null, fetch(textUrl)→string|null }`. `SOURCES` (registry.ts:134-141) is ordered best-first: openstax, siyavula, standard-ebooks, gutenberg, wikisource, internet-archive. Public entry points: `discoverFullText` (first hit), `discoverAllFullText` (all hits ranked), `fetchFullText` (one fetch), `fetchFirstAvailable` (fetch-fallback w/ wall-clock budget), `findFullText` (discover+fetch convenience).
...
**Matching:** `matching.ts` provides `normalize`, `authorLastName`, `scoreTitleAuthor`, and the shared `BROWSER_UA`. `gutenberg.ts` carries its OWN private copies of `normalize`/`authorLastName`/`BROWSER_UA` (gutenberg.ts:37-63) — duplication/drift (see findings). Matching is deliberately conservative: a wrong full text is worse than none.
...
1. `discoverAllFullText({ title, authors })` (registry.ts:176) → loops `SOURCES` best-first, calls each `source.discover` (registry.ts:185), collects every `{source, sourceId, textUrl}` hit. Called at ingest-book-fulltext.ts:58.
2. `fetchFirstAvailable(discovered, { budgetMs: 40000 })` (registry.ts:223, called at ingest-book-fulltext.ts:83) → iterates candidates in priority order, calls `fetchFullText` (registry.ts:201 → `src.fetch`), returns first usable text within the wall-clock budget.
...
|------|--------|----------|
| `discoverAllFullText` / `fetchFirstAvailable` | DONE | registry.ts:176,223; wired ingest-book-fulltext.ts:58,83 |
| `fetchFullText` | DONE | registry.ts:201; called internally by `fetchFirstAvailable` (registry.ts:232) |
| `discoverFullText` | DEAD | registry.ts:148; only caller is `findFullText` (registry.ts:248); no external importer (Grep) |
| `findFullText` | DEAD | registry.ts:244; zero importers repo-wide (Grep `findFullText`) — "for scripts/smoke tests" per docstring, none present |
...
| matching helpers | DONE | matching.ts; imported by libretexts/siyavula/standard-ebooks/internet-archive/wikisource |
...
| `computeDedupKey` | DONE | book-dedup.ts:150; extract/route.ts:77 |
...
No STUBs found — every adapter has a real fetch/parse body. The only DEAD units are the single-hit convenience wrappers `discoverFullText` + `findFullText` (the worker uses the all-hits variants instead).
...
**Imports in:** `cheerio` (openstax, libretexts, siyavula, standard-ebooks, wikisource, commentary-parser); `youtube-transcript` (dynamic import, transcript.ts:72); `@/generated/client` Book type (google-books.ts:1 — imported but UNUSED, see findings). `book-dedup`, `text-processing`, `matching`, `bible-books` are pure/dependency-free.
...
Q-13-001  [LOW]  Dead convenience wrappers `discoverFullText` + `findFullText`  — src/lib/sources/registry.ts:148,244
...
Q-13-002  [LOW]  Duplicated matching + UA helpers in gutenberg.ts diverge from the shared `matching.ts`  — src/lib/sources/gutenberg.ts:37-139
